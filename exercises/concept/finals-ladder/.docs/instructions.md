...
The stub gives you a `TEAM` class, and a `FINALS_LADDER` class whose
routines you fill in.

## 1. Who is above whom?

`higher` takes two teams and answers whether the first belongs above the
second. More points comes first. Teams level on points are separated by
goal difference, higher first.
...
`premiers` takes the teams in any order and answers the name of the team
at the top. With no teams at all, the answer is `""`.
...
`shorter` takes two strings and answers whether the first belongs before
the second: shorter first, and alphabetical for strings of the same
length. `shortest_first` takes an array of strings and answers them in
that order.
...
This is the same sorting routine as task 3, with a different rule handed
