...
- Every loop here is driven by an iterator, so none of them needs a
  counter variable or an `until!`.
...
- Each time round, compare this day against it and keep the larger. `.max`
  does that in one step.

## 3. The busy days

- Count rather than add: an `if` deciding whether to add one to the count.
- Ask the iterator once each time round. Writing `counts.elt!` twice in
  one body walks the array twice over.

## 4. Hides to check

- Nothing is being walked here, so `elt!` is no use. `1.upto!(days)`
  counts for you.
...
- The answer is a position, not a value, so walk the positions:
  `0.upto!(counts.size - 1)`.
- Start the answer at `-1`. If nothing is found, that is already what
  should be returned.
