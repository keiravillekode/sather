...
- Every name here ends in `!`, and every one hands values over with
  `yield` rather than `return`.
- Mark the arguments `once`. Nothing here wants an argument re-evaluated
  every turn.
- Test one by writing a loop around it. An iterator cannot be called on
  its own, so there is nothing to print without a `loop`.

## 1. The markers

- A `loop` inside the iterator, driven by `1.upto!(count)`, yielding a
  distance worked out from the number.
...
- The order matters: `quit` before the `yield`, or `"end"` gets handed
  over before you stop.
- Nothing is needed for an array with no `"end"` in it. The `elt!` runs
  out and the iterator is over.
...
- Two iterators inside your own iterator: `1.up!` for the number and
  `elt!` for the sighting. This is the pairing from `duet-practice`, now
  inside something you wrote.
