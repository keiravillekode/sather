...
`sort` works on anything that already knows how to compare itself —
numbers and strings do, because they have an `is_lt`.
...
second. It calls that routine as often as it needs to and puts the array
in the order the answers imply.

This is what `understudy` was for. The library knows how to sort and
nothing about your idea of order; the bound routine is where the two meet.
...
```sather
   cheaper(a, b : SEAT) : BOOL is
      if a.price /= b.price then
         return a.price < b.price;    -- cheaper seats come first
      end;
      return a.row < b.row;           -- same price: front rows first
   end;
```

The first `if` settles it whenever the prices differ. Only when they are
equal does the second line get a say. Writing the tie-breaker first would
mean the price never mattered.
...
```sather
   seats.insertion_sort_by(bind(cheaper(_, _)));   -- seats is now sorted
```
...
```sather
   ordered ::= seats.copy;
   ordered.insertion_sort_by(bind(cheaper(_, _)));
   return ordered;
```
